...
def compile_model(job, save_fn, model):
    ...
    while index < len(model.modules):
        ...
        if isinstance(module, ExLlamaV2Embedding):
            d = get_f_module(job, module)
            out_dict.update(d)
            current_size += _dsize(d)

        if isinstance(module, ExLlamaV2Attention):
            d = get_f_module(job, module.input_layernorm)
            out_dict.update(d)
            current_size += _dsize(d)
            d = get_q_module(job, module.q_proj)
            out_dict.update(d)
            current_size += _dsize(d)
            d = get_q_module(job, module.k_proj)
            out_dict.update(d)
            current_size += _dsize(d)
            d = get_q_module(job, module.v_proj)
            out_dict.update(d)
            current_size += _dsize(d)
            d = get_q_module(job, module.o_proj)
            out_dict.update(d)
            current_size += _dsize(d)

        if isinstance(module, ExLlamaV2MLP):
            d = get_f_module(job, module.post_attention_layernorm)
            out_dict.update(d)
            current_size += _dsize(d)
            d = get_q_module(job, module.gate_proj)
            out_dict.update(d)
            current_size += _dsize(d)
            d = get_q_module(job, module.up_proj)
            out_dict.update(d)
            current_size += _dsize(d)
            d = get_q_module(job, module.down_proj)
            out_dict.update(d)
            current_size += _dsize(d)
        ...
        if isinstance(module, ExLlamaV2MoEMLP):
            ...
            out_dict.update(d)
            current_size += _dsize(d)
            d = get_f_module(job, module.gate)
            out_dict.update(d)
            current_size += _dsize(d)
            for i in range(model.config.num_experts):
                d = get_q_module(job, module.w1[i])
                out_dict.update(d)
                current_size += _dsize(d)
                d = get_q_module(job, module.w3[i])
                out_dict.update(d)
                current_size += _dsize(d)
                d = get_q_module(job, module.w2[i])
                out_dict.update(d)
                current_size += _dsize(d)
        ...
        if current_size > shard_bytes or index == len(model.modules):
            ...
            if len(save_dict) == 0:
                print(" ## Error: Unable to fit output tensor in single shard.")
                os._exit(0)
    ...
    if job["compile_full"] is not None:
        ...
        if len(bin_files) > 0:
            print(" !! Ignoring *.bin files in source dir")
