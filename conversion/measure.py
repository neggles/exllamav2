...
def embeddings(job, save_fn, model, measure=False):
    ...
    embeddings_dict = {
        f"row.{i:05}": hidden_state[i : i + 1, :, :].contiguous() for i in range(hidden_state.shape[0])
    }
...
def test_quant(source: ExLlamaV2Linear, lq: AdaptiveGPTQ, qparams: list):
    variants = []
    variants_bits = []

    original = nn.Linear(
        source.in_features, source.out_features, source.has_bias, device="meta", dtype=torch.float16
    )
    original.weight = nn.Parameter(source.linear.weight.clone())
    if source.has_bias:
        original.bias.weight = nn.Parameter(source.linear.bias.clone())
    ...
    for qp in qparams:
        ...
        total_bits = qp.total_bits(
            quantized.weight.T.shape, original.bias.weight.shape if source.has_bias else None
        )
        variants_bits.append(total_bits)

        numel = quantized.weight.numel()
        if source.has_bias:
            numel += original.bias.numel()
...
def measure_attn(module, hidden_states, target_states, quantizers, cache, attn_params):
    ...
    for q, k, v, o in qmaps:
        if q != q_:
            module.q_proj.linear.weight = nn.Parameter(options_q[q].weight.cuda())
        if k != k_:
            module.k_proj.linear.weight = nn.Parameter(options_k[k].weight.cuda())
        if v != v_:
            module.v_proj.linear.weight = nn.Parameter(options_v[v].weight.cuda())
        if o != o_:
            module.o_proj.linear.weight = nn.Parameter(options_o[o].weight.cuda())
        (q_, k_, v_, o_) = (q, k, v, o)
...
def measure_mlp(module, hidden_states, target_states, quantizers, cache, attn_params):
    ...
    for g, u, d in qmaps:
        if g != g_:
            module.gate_proj.linear.weight = nn.Parameter(options_g[g].weight.cuda())
        if u != u_:
            module.up_proj.linear.weight = nn.Parameter(options_u[u].weight.cuda())
        if d != d_:
            module.down_proj.linear.weight = nn.Parameter(options_d[d].weight.cuda())
        (g_, u_, d_) = (g, u, d)
...
def measure_moe_mlp(module, hidden_states, target_states, quantizers, cache, attn_mask):
    ...
    for g, u, d in qmaps:
        for i in range(num_experts):
            if g != g_:
                module.w1[i].linear.weight = nn.Parameter(options_g[i][g].weight.cuda())
            if u != u_:
                module.w3[i].linear.weight = nn.Parameter(options_u[i][u].weight.cuda())
            if d != d_:
                module.w2[i].linear.weight = nn.Parameter(options_d[i][d].weight.cuda())
        (g_, u_, d_) = (g, u, d)
...
def measure_quant(job, save_fn, model):
    ...
    exp_measurement = {"measurement": job["measurement"], "last_module_idx": job["last_module_idx"]}
