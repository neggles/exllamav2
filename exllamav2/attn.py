...
try:
    ...
    is_ampere_or_newer_gpu = any(
        torch.cuda.get_device_properties(i).major >= 8 for i in range(torch.cuda.device_count())
    )
...
class ExLlamaV2Attention(ExLlamaV2Module):
    layer_idx: int
    input_layernorm: Optional[ExLlamaV2RMSNorm | ExLlamaV2LayerNorm]
    q_proj: Optional[ExLlamaV2Linear]
    k_proj: Optional[ExLlamaV2Linear]
    v_proj: Optional[ExLlamaV2Linear]
    ...
    class Params:
        ...
        def get_attn_masks(self, device):
            if self.attn_masks is None:
                ...
                self.attn_masks = [
                    (safe_move_tensor(m, device) if m is not None else None) for m in self.attn_masks
                ]
            return self.attn_masks

        def build_single_attn_mask(self, batch_size, seq_len, past_len, device, input_mask):
            attn_mask = torch.zeros(
                (batch_size, 1, seq_len, past_len + seq_len), dtype=torch.float16, device=device
            )
            ...
            if input_mask is not None:
                ...
                attn_mask[:, :, :, :min_mask_width] = torch.minimum(
                    attn_mask[:, :, :, :min_mask_width], input_mask_part
                )
        ...
        def build_attn_mask(self, device):
            ...
            if self.input_mask is None and self.seq_len == 1:
                return None
            return self.build_single_attn_mask(
                self.batch_size, self.seq_len, self.past_len, device, self.input_mask
            )
        ...
        def build_attn_masks(self, device):
            ...
            for i, past_len in enumerate(self.past_lens):
                if self.input_mask is None and self.seq_len == 1:
                    ...
                    attn_masks.append(
                        self.build_single_attn_mask(1, self.seq_len, past_len, device, self.input_mask[i])
                    )
            return attn_masks
    ...
    def __init__(self, model, key, layer_idx):
        ...
        self.q_proj = ExLlamaV2Linear(
            model,
            key + ".self_attn.q_proj",
            hidden_size,
            self.model.config.num_attention_heads * self.model.config.head_dim,
            self.model.config.attention_bias_qkv,
        )
        self.k_proj = ExLlamaV2Linear(
            model,
        ...
        self.o_proj = ExLlamaV2Linear(
            model,
            key + ".self_attn.o_proj",
            self.model.config.num_attention_heads * self.model.config.head_dim,
            hidden_size,
            self.model.config.attention_bias_o,
        )

        self.submodules = [self.input_layernorm, self.q_proj, self.k_proj, self.v_proj, self.o_proj]

    def numel(self):
        return self.q_proj.numel() + self.k_proj.numel() + self.v_proj.numel() + self.o_proj.numel()
    ...
    def load(self):
        ...
        if self.q_proj.is_quant():
            assert (
                self.k_proj.is_quant() and self.v_proj.is_quant() and self.o_proj.is_quant()
            ), "Partially quantized attention layer"
    ...
    def scratch_space_fixed(self):
        return self.temp_state_size() + self.temp_dq_size()
    ...
    def temp_state_size(self):
        return (
            self.model.config.max_input_len
            * self.model.config.max_batch_size
            * self.model.config.num_attention_heads
            * self.model.config.head_dim
            * 2
            + 128
    ...
    def temp_attn_size(self):
        ...
        if has_flash_attn and not self.model.config.no_flash_attn:
            eff = (
                self.model.config.max_attention_size**0.5 / 190
            )  # based on supposed memory savings listed in flash-attn repo + some fudging
            att_max //= eff
    ...
    def forward(
        ...
        if loras is None or self.temp_lora_size == 0:
            ...
            pass_lora_temp = torch.empty(
                (self.temp_lora_size,), dtype=torch.half, device=hidden_states.device
            )
        ...
        if cache is None or isinstance(cache, ExLlamaV2CacheBase):
            ...
            if self.model.config.no_flash_attn or not has_flash_attn or not attn_params.is_causal():
                ...
                attn_output = attn_output.reshape(
                    (batch_size, q_len, self.model.config.num_attention_heads * self.model.config.head_dim)
                )
        ...
        # Multiple caches
            ...
            for i in range(len(cache)):
                ...
                if attn_masks[i] is not None:
                    attn_weights = attn_weights + attn_masks[i]
                attn_weights = nn.functional.softmax(attn_weights, dim=-1, dtype=torch.float16)
        ...
        ext_c.q_attn_forward_2(
            self.q_handle, hidden_states, attn_output, batch_size, q_len, pass_loras, pass_lora_temp
        )
    ...
    def update_loras(self):
        ...
        self.temp_lora_size = (
            temp_lora_size * self.model.config.max_batch_size * self.model.config.max_input_len
        )
