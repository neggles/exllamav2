...
class ExLlamaV2CacheBase(ABC):
    ...
    def update_cache_tensors(self):
        for k, v in self.model.cache_map.items():
            ...
            if self.key_states[k] is not None:
                if str(self.key_states[k].device) == v:
                    continue
    ...
    def copy_states(
        ...
        for i in range(num_hidden_layers):
            source_view_k = (
                self.key_states[i].narrow(0, from_row, from_rows).narrow(2, from_column, from_columns)
            )
            source_view_v = (
                self.value_states[i].narrow(0, from_row, from_rows).narrow(2, from_column, from_columns)
            )
...
class ExLlamaV2Cache_8bit(ExLlamaV2CacheBase):
    ...
    def get_kv_state(self, layer_idx: int, batch_size: int, offset: int, width: int) -> tuple[Tensor, Tensor]:
        ...
        if width > 0:
            ext_c.fp8_to_fp16(self.key_states[layer_idx], temp_key_state, batch_size, offset, width)
        if width > 0:
            ext_c.fp8_to_fp16(self.value_states[layer_idx], temp_value_state, batch_size, offset, width)
    ...
    def store_kv_state(self, layer_idx: int, batch_size: int, offset: int, width: int):
        ...
        temp_key_state, temp_value_state = self.temp_tensors[device]
        if width > 0:
            ext_c.fp16_to_fp8(temp_key_state, self.key_states[layer_idx], batch_size, offset, width)
        if width > 0:
