...
class ExLlamaV2BaseGenerator:
    ...
    def generate_simple(
        ...
        ids, position_offsets = self.tokenizer.encode(
            prompt, encode_special_tokens=encode_special_tokens, return_offsets=True, add_bos=add_bos
        )
        if batch_size == 1:
        ...
        for i in range(num_tokens):
            ...
            token, _, _, _, _ = ExLlamaV2Sampler.sample(
                logits,
                gen_settings,
                self.sequence_ids,
                random.random(),
