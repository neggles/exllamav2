...
class ExLlamaV2Sampler:
    ...
    def sample(
        ...
        assert prefix_token is None or prefix_token.shape == (
            batch_size,
            1,
        ...
        if (
            ...
            or settings.token_frequency_penalty != 0.0
            or settings.token_presence_penalty != 0.0
        ):
            ext_c.apply_rep_penalty(
                sequence_ids[:, :],
        ...
        end_filter = False
        if len(settings.filters) > 0 and output_tokens[0].item() in end_tokens:
            end_filter = True
