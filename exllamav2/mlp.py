...
class ExLlamaV2MLP(ExLlamaV2Module):
    ...
    def __init__(self, model, key, layer_idx):
        ...
        self.submodules = [self.post_attention_layernorm, self.gate_proj, self.up_proj, self.down_proj]

    def numel(self):
        return self.gate_proj.numel() + self.up_proj.numel() + self.down_proj.numel()
    ...
    def scratch_space_fixed(self):
        return self.temp_state_size() + self.temp_a_size() + self.temp_b_size() + self.temp_dq_size()
    ...
    def temp_dq_size(self):
        return max(self.gate_proj.temp_dq_size(), self.up_proj.temp_dq_size(), self.down_proj.temp_dq_size())
    ...
    def forward(
        ...
        ext_c.q_mlp_forward_(
            self.q_handle, hidden_states.view(-1, hidden_states.shape[-1]), pass_loras, pass_lora_temp
        )
    ...
    def update_loras(self):
        ...
        temp_lora_size = ext_c.q_mlp_set_loras(
            self.q_handle,
            gate_proj_lora_a,
            gate_proj_lora_b,
            up_proj_lora_a,
