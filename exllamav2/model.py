...
class ExLlamaV2DeviceTensors:
    ...
    def prepare(self, scratch):
        ...
        if scratch:
            self.scratch = torch.empty(
                (self.scratch_bytes // 2,), dtype=torch.half, device=_torch_device(self.device_idx)
            )
    ...
    def prepare_sincos(self):
        ...
        if alpha != 1.0:
            base *= alpha ** (self.model.config.head_dim / (self.model.config.head_dim - 2))
...
class ExLlamaV2:
    ...
    def __init__(self, config: ExLlamaV2Config, lazy_load=False):
        ...
        for layer_idx in range(self.config.num_hidden_layers):
            ...
            for m in self.modules[-1].submodules:
        ...
        self.modules.append(
            ExLlamaV2Linear(self, "lm_head", self.config.hidden_size, self.config.vocab_size, False)
        )
        self.modules_dict[self.modules[-1].key] = self.modules[-1]
    ...
    def set_device_map(self, allocation, embed_cpu=True):
        ...
        return [
            (ab - rb - rba) / 1024**3
            for (ab, rb, rba) in zip(allocation_bytes, reserve_bytes, reserve_bytes_attn)
        ]
            ...
            if not lazy:
                for idx, module in enumerate(self.modules):
                    if callback is not None:
                        callback(idx, len(self.modules))
                    if callback_gen is not None:
                        yield from callback_gen(idx, len(self.modules))

                    module.load()

                if callback is not None:
                    callback(len(self.modules), len(self.modules))
                if callback_gen is not None:
                    yield from callback_gen(len(self.modules), len(self.modules))
    ...
    def load_autosplit_gen(
        self, cache, reserve_vram=None, last_id_only=False, callback=None, callback_gen=None
    ):
                ...
                while True:
                    ...
                    if current_device > last_touched_device:
                        self.device_tensors.append(
                            ExLlamaV2DeviceTensors(self, current_device, scratch_fixed)
                        )
                        ...
                        reserved_vram_tensors.append(
                            torch.empty((b,), dtype=torch.int8, device=_torch_device(current_device))
                        )
                        minimum_reserve_tensor = torch.empty(
                            (minimum_reserve_vram,), dtype=torch.int8, device=_torch_device(current_device)
                        )
                    ...
                    try:
                        ...
                        hidden_state = module.forward(
                            hidden_state, cache=cache, attn_params=attn_params, past_len=past_len, loras=loras
                        )
                        fail = False
                        ...
                        test = 0
                        if (
                            e.__class__.__name__ == "OutOfMemoryError"
                    ...
                    if fail:
                        ...
                        if minimum_reserve_tensor is not None:
                            del minimum_reserve_tensor
                        minimum_reserve_tensor = None
    ...
    def get_device_tensors(self, device_idx, scratch=True):
        ...
        if not tensors.ready:
            tensors.prepare(scratch)
        return tensors
    ...
    def forward(
        ...
        if cache is None or not isinstance(cache, ExLlamaV2CacheBase):
            ...
            result, last_state = self._forward(
                input_ids=input_ids,
                cache=cache,
        ...
        assert (
            past_len + q_len <= cache.max_seq_len
