...
def _tsize(st, key):
    ...
    if dtype == "I32":
        ...
        raise ValueError(f"Unexpected datatype {dtype}: {key}")
    ...
    def load_weight(self, override_key=None):
        ...
        for key in keys:
            ...
            if key + ".q_weight" in self.model.config.tensor_file_map:
                qtensors = self.load_multi(
                    key,
                    ["q_weight", "q_invperm", "q_scale", "q_scale_max", "q_groups", "q_perm", "bias"],
            ...
            if key + ".qweight" in self.model.config.tensor_file_map:
                qtensors = self.load_multi(
                    key, ["qweight", "qzeros", "scales", "g_idx", "bias"], override_key=override_key
                )
    ...
    def weight_footprint(self):
        if self.footprint == -1:
            ...
            for key in keys:
                ...
                if key + ".q_weight" in self.model.config.tensor_file_map:
                    self.footprint = self.load_multi(
                        key,
                        ["q_weight", "q_invperm", "q_scale", "q_scale_max", "q_groups", "q_perm", "q_perm"],
                        measure=True,
                    )
                ...
                # GPTQ
                    ...
                    self.footprint = self.load_multi(
                        key, ["qweight", "qzeros", "scales", "g_idx"], measure=True
                    )
