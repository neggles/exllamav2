...
class ExLlamaV2MoEMLP(ExLlamaV2Module):
    ...
    def __init__(self, model, key, layer_idx):
        ...
        self.w1 = [
            ExLlamaV2Linear(
                model, key + f".block_sparse_moe.experts.{e}.w1", hidden_size, intermediate_size, False
            )
            for e in range(self.num_experts)
        ]
        self.w2 = [
            ExLlamaV2Linear(
                model, key + f".block_sparse_moe.experts.{e}.w2", intermediate_size, hidden_size, False
            )
            for e in range(self.num_experts)
        ]
        self.w3 = [
            ExLlamaV2Linear(
                model, key + f".block_sparse_moe.experts.{e}.w3", hidden_size, intermediate_size, False
            )
            for e in range(self.num_experts)
        ]
        self.gate = ExLlamaV2Linear(
            model, key + ".block_sparse_moe.gate", hidden_size, self.num_experts, False, pad32=False
        )

        self.submodules = [self.post_attention_layernorm, self.gate] + self.w1 + self.w2 + self.w3

    def numel(self):
        return sum(wt.numel() for wt in self.w1 + self.w2 + self.w3)
    ...
    def load(self):
        ...
        if self.w1[0].is_quant():
            ...
            self.q_handle = ext_c.make_q_moe_mlp(
                ...
                if self.post_attention_layernorm.bias is not None
                else ext.none_tensor,
    ...
    def weight_footprint(self):
        return (
            self.post_attention_layernorm.weight_footprint()
    ...
    def temp_dq_size(self):
        return max(self.w1[0].temp_dq_size(), self.w2[0].temp_dq_size(), self.w3[0].temp_dq_size())
    ...
    def forward(
        ...
        if (
            self.q_handle is None
            or intermediates
            or batch_size * sequence_length > 4
            or self.num_experts not in [4, 8]
            or (loras is not None and len(loras) > 0)
        ):
    ...
    def forward_torch(
        ...
        if intermediates:
            result = {"post_norm": hidden_states}
        ...
        expert_mask = torch.nn.functional.one_hot(selected_experts, num_classes=self.num_experts).permute(
            2, 1, 0
        )
        ...
        for expert_idx in range(self.num_experts):
            ...
            if top_x.shape[0] == 0:
                continue  # Skip experts that weren't selected at all
            ...
            current_hidden_states = (
                F.gelu(gate) if self.model.config.architecture == "Gemma" else F.silu(gate)
            ) * up
            if intermediates:
                result[f"pre_down.{expert_idx}"] = current_hidden_states
