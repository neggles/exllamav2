...
parser.add_argument(
    ...
    "--eval_token",
    action="store_true",
    help="Evaluate perplexity on token-by-token inference using cache",
)
parser.add_argument(
    "-e8",
    "--eval_token_8bit",
    action="store_true",
    help="Evaluate perplexity on token-by-token inference using 8-bit cache",
)
parser.add_argument(
    "-eb",
    "--eval_bos",
    action="store_true",
    help="Add BOS token to every row in perplexity test (required by Gemma and maybe other models.)",
)
...
parser.add_argument(
    ...
    "--prompt_speed",
    action="store_true",
    help="Test prompt processing (batch) speed over context length",
)
parser.add_argument(
    "-s", "--speed", action="store_true", help="Test raw generation speed over context length"
)
parser.add_argument(
    "-mix",
    "--mix_layers",
    type=str,
    help="Load replacement layers from secondary model. Example: --mix_layers 1,6-7:/mnt/models/other_model",
)
...
parser.add_argument(
    ...
    "--stream_layers",
    action="store_true",
    help="Load model layer by layer (perplexity evaluation only)",
)
parser.add_argument(
    "-sp",
    "--standard_perplexity",
    choices=["wiki2"],
    help="Run standard (HF) perplexity test, stride 512 (experimental)",
)
parser.add_argument(
    "-rr",
    "--rank_reduce",
    type=str,
    help="Rank-reduction for MLP layers of model, in reverse order (for experimentation)",
)
...
def main(args: argparse.Namespace):
    ...
    model, tokenizer = model_init.init(
        args, allow_auto_split=True, skip_load=args.stream_layers, benchmark=True
    )
    cache = None
    ...
    if args.mix_layers:
        ...
        for interval in intervals:
            ...
            for idx in range(a, b + 1):
                ...
                for k in remove:
                    del model.config.tensor_file_map[k]
                for k in replace:
                    model.config.tensor_file_map[k] = extra_config.tensor_file_map[k]
                # for k in reload:
    ...
    if args.prompt:
        with torch.inference_mode():
            ...
            output = generator.generate_simple(
                args.prompt, settings, args.tokens, token_healing=True, add_bos=not args.prompt_no_bos
            )
        ...
        print(
            f" -- Response generated in {total_gen:.2f} seconds, {args.tokens} tokens, {args.tokens / total_gen:.2f} tokens/second (includes prompt eval.)"
        )
    ...
    if args.eval_dataset or args.standard_perplexity:
        with torch.inference_mode():
            ...
            if args.stream_layers:
                ...
                if cache is None:
                    cache = (
                        ExLlamaV2Cache(model, max_seq_len=eval_length)
            ...
            if args.eval_token:
                if args.standard_perplexity:
                    print(" !! Note, can't evalutate token perplexity on standard test")
                else:
                    print(" -- Inference (token)", end="")
                    sys.stdout.flush()
            ...
            if args.eval_token_8bit:
                if args.standard_perplexity:
                    ...
                    print(" -- Inference (token, 8-bit cache)", end="")
                    sys.stdout.flush()
