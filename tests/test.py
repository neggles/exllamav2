...
def test_gen_batch(max_new_tokens):
    ...
    prompts = [
        "Here's how to create a powerful love potio",
        "For once,",
    ...
    print()
    print(
        f"Response generated in {time_total:.2f} seconds, {max_new_tokens} tokens, throughput {4 * max_new_tokens / time_total:.2f} tokens/second"
    )
    ...
    for i in range(max_new_tokens):
        ...
        for j in range(len(input_ids)):
            token, _, _ = ExLlamaV2Sampler.sample(
                logits[j : j + 1, :, :], settings, input_ids[j], r, tokenizer
            )
